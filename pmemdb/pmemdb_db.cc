...
// merge operator disabled by default due to link error
  ...
  opt.env = rocksdb::NewDCPMMEnv(rocksdb::DCPMMEnvOptions());
  opt.dcpmm_kvs_enable = true;
  opt.dcpmm_kvs_level = 0;
  //opt.dcpmm_kvs_mmapped_file_fullpath = db_path + "/kvs";
  opt.dcpmm_kvs_mmapped_file_fullpath = db_path;
  opt.dcpmm_kvs_mmapped_file_size = 4*(1ull<<30);
  opt.dcpmm_kvs_value_thres = 64; // minimal size to do kv sep
  opt.dcpmm_compress_value = false;

  //Optimized mmap read for pmem
  /*
  opt.use_mmap_read = true;
  opt.cache_index_and_filter_blocks_for_mmap_read = true;
  rocksdb::BlockBasedTableOptions bbto;
  bbto.block_size = 256;
  opt.table_factory.reset(rocksdb::NewBlockBasedTableFactory(bbto));
*/
  ...
  std::string data_dir = db_path + "/db";
  rocksdb::Status s;
  ...
  if (cf_descs.empty()) {
    ...
    s = rocksdb::DB::Open(opt, data_dir, &db_);
  } else {
    //s = rocksdb::DB::Open(opt, db_path, cf_descs, &cf_handles, &db_);
    s = rocksdb::DB::Open(opt, data_dir, cf_descs, &cf_handles, &db_);
  }
  if (!s.ok()) {
...
const bool registered = DBFactory::RegisterDB("pmemdb", NewRocksdbDB);
